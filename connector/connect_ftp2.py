...
class ConnectFTP2:
    ...
    @staticmethod
    def download_files(ftp_endpoint:str, ftp_path:str, pattern:str,
                        local_path:str):
        '''
        anonymous user login FTP site
        Download all files at a time, excluding sub-directories
        local file would be covered if that exists.
        '''
        dir_names = [local_path,] + ftp_path.split('/')
        current_local_path = os.path.join(*dir_names)
        Dir(current_local_path).init_dir()
        contents = ConnectFTP2.parse_contents(
            ftp_endpoint,  ftp_path, pattern
        )
